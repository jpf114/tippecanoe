...
tippecanoe-db \
  --postgis "localhost:5432:mydb:postgres:password:mytable:geom" \
  --mongo "localhost:27017:tilerender:admin:password:admin:mytiles_dev" \
...
| `--postgis-progress-count` | 否 | 否 | 开启精确进度：执行 `COUNT(*)` 获取总量；默认关闭以减少扫描开销 |
...
| 参数 | 必选 | 默认值 | 说明 |
|------|------|--------|------|
| `--mongo-batch-size` | 否 | 100 | 批量写入大小（10-1000） |
| `--mongo-pool-size` | 否 | 10 | 连接池大小（上限 50） |
...
async function getMetadata() {
  ...
  // v2.0: 元数据为 BSON 文档，vector_layers 和 tilestats 为 JSON 字符串
  return {
    name: meta.name,
    bounds: meta.bounds,
    center: meta.center,
    ...
    vector_layers: meta.vector_layers ? JSON.parse(meta.vector_layers) : [],
    tilestats: meta.tilestats ? JSON.parse(meta.tilestats) : null
  };
}
```
...
```json
{
  ...
  "vector_layers": "[{\"id\":\"china\",\"fields\":{\"name\":\"String\",\"code\":\"String\"},\"minzoom\":5,\"maxzoom\":10}]",
  "tilestats": "{\"layerCount\":1,\"layers\":[{\"layer\":\"china\",\"count\":14182,\"geometry\":\"Polygon\",\"attributeCount\":2,\"attributes\":[...]}]}",
  "attribution": "",
...
| vector_layers | string (JSON) | **v2.0 新增**：矢量图层描述及属性 schema |
| tilestats | string (JSON) | **v2.0 新增**：属性统计信息 |
| attribution | string | 版权声明（如有） |
| generator | string | 生成工具信息 |
| collection | string | 对应的瓦片集合名 |
| timestamp | int64 | 生成时间戳（毫秒） |

> **注意：** v2.0 版本起，元数据使用 BSON 原生构建，`vector_layers` 和 `tilestats` 字段以 JSON 字符串形式存储，读取时需解析。
...
**Q: 如何优先用主键分片，而不是 ctid？**

A: 可显式指定：
1. `--postgis-shard-key id`（业务字段）
2. `--postgis-shard-mode range`（优先范围分片）

若使用 `auto`（默认），系统会按 `range -> key-hash -> ctid -> 单线程` 逐级回退。
...
| 退出码 | 常量 | 含义 |
|--------|------|------|
| 0 | EXIT_SUCCESS / EXIT_OK | 正常退出 |
| 100 | EXIT_INCOMPLETE | 数据不完整 |
| 101 | EXIT_ARGS | 参数错误（如 minzoom > maxzoom） |
| 102 | EXIT_CLOSE | 文件/数据库关闭失败 |
| 103 | EXIT_CSV | CSV 解析错误 |
| 104 | EXIT_EXISTS | 输出文件已存在（未使用 -f） |
| 105 | EXIT_FILTER | 过滤器错误 |
| 106 | EXIT_IMPOSSIBLE | 无法完成的操作 |
| 107 | EXIT_JSON | JSON 解析错误 |
| 108 | EXIT_MEMORY | 内存不足 |
| 109 | EXIT_MVT | MVT 编码错误 |
| 110 | EXIT_NODATA | 无数据 |
| 111 | EXIT_OPEN | 文件/数据库打开失败 |
| 112 | EXIT_PROTOBUF | Protobuf 编码错误 |
| 113 | EXIT_PTHREAD | 线程错误 |
| 114 | EXIT_READ | 读取错误 |
| 115 | EXIT_SEEK | 文件定位错误 |
| 116 | EXIT_SQLITE | SQLite 错误 |
| 117 | EXIT_STAT | 文件状态查询失败 |
| 118 | EXIT_UNLINK | 文件删除失败 |
| 119 | EXIT_UTF8 | UTF-8 编码错误 |
| 120 | EXIT_WRITE | 写入错误 |
| 121 | EXIT_MONGO | MongoDB 写入错误（包括有瓦片被丢弃） |

> **v2.0 新增：** 当 MongoDB 写入过程中有瓦片被丢弃时（`total_discarded > 0`），默认退出码为 `EXIT_MONGO (121)`；可用 `--mongo-no-fail-on-discard` 切换为仅告警。
...
#### 7.2.1 字节级一致性（主线 `tippecanoe` 对比）

当前版本已验证：在相同数据源和参数下，`tippecanoe-db` 与主线 `tippecanoe` 可达到 **tile 坐标、字节总量、payload hash 全一致**。

**建议对比模式：**

- 使用 `--postgis-shard-mode none` 固定读取顺序，避免并行调度引入的非业务性顺序扰动。
- 使用同一份源数据进行对比：  
  - `tippecanoe-db` 直接读 PostGIS  
  - 主线 `tippecanoe` 读同源导出的 NDJSON (`row_to_json(...)::text`)
...
psql "host=localhost port=5432 dbname=TippTest user=postgres password=pg" \
  -c "\copy (SELECT row_to_json(t)::text FROM (SELECT geom,id,name,adcode FROM china) t) TO 'build/china.ndjson'"

# 3) 生成主线版本
./tippecanoe -zg -o build/consistency_native_feature.mbtiles build/china.ndjson

# 4) 比较 tile 总数与 payload 总字节
sqlite3 build/consistency_db_exact.mbtiles "SELECT COUNT(*), SUM(LENGTH(tile_data)) FROM tiles;"
sqlite3 build/consistency_native_feature.mbtiles "SELECT COUNT(*), SUM(LENGTH(tile_data)) FROM tiles;"

# 5) 比较 payload 全量 hash（按 z/x/y 排序后）
sqlite3 build/consistency_db_exact.mbtiles "SELECT zoom_level,tile_column,tile_row,hex(tile_data) FROM tiles ORDER BY 1,2,3;" | sha256sum
sqlite3 build/consistency_native_feature.mbtiles "SELECT zoom_level,tile_column,tile_row,hex(tile_data) FROM tiles ORDER BY 1,2,3;" | sha256sum
```

**一致性判定标准：**

1. `tiles` 行数一致
2. `SUM(LENGTH(tile_data))` 一致
3. 上述全量 hash 一致
...
| 分区表并行受限 | ctid 在分区表上不可用 | 配置 `--postgis-shard-key` + `--postgis-shard-mode range/key` |
| 自定义 SQL 并行受限 | 子查询可能无法使用 ctid | 优先使用 `shard-key`，否则自动降级单线程 |
...
| `--postgis-user` | — | 条件必选 | — | 分字段模式下必填之一 |
| `--postgis-password` | — | 条件必选 | — | 分字段模式下必填之一 |
| `--postgis-table` | — | 条件必选 | — | 与 `--postgis-sql` 互斥（二选一） |
| `--postgis-geometry-field` | — | 否 | `geometry` | 与 `--postgis-table` 配套 |
...
| `--postgis-progress-count` | — | 否 | 否 | 开启精确总量统计（额外 `COUNT(*)`） |
...
| `--mongo-username` | — | 条件必选 | — | 分字段模式下必填之一 |
| `--mongo-password` | — | 条件必选 | — | 分字段模式下必填之一 |
| `--mongo-auth-source` | — | 条件必选 | — | 分字段模式下必填之一 |
| `--mongo-collection` | — | 条件必选 | — | 分字段模式下必填之一 |
| `--mongo-batch-size` | — | 否 | 100 | 范围 10-1000 |
| `--mongo-pool-size` | — | 否 | 10 | 上限 50 |
...
| 参数 | 短选项 | 必选 | 默认值 | 说明 |
|------|--------|------|--------|------|
| `--maximum-zoom` | `-z` | 否 | 14 | 最大缩放级别 |
| `--minimum-zoom` | `-Z` | 否 | 0 | 最小缩放级别 |
| `--output` | `-o` | 否 | — | 可选 MBTiles 输出；与 Mongo 可并行使用 |
| `--exclude` | `-x` | 否 | — | 排除属性字段 |
| `--include` | `-y` | 否 | — | 仅包含属性字段 |
| `--drop-densest-as-needed` | — | 否 | 否 | 控制瓦片大小常用 |
| `--extend-zooms-if-still-dropping` | — | 否 | 否 | 仍超限时扩展 zoom |
| `--quiet` | `-q` | 否 | 否 | 静默模式 |

### 9.4 最小必选参数组合

- **PostGIS 输入**：`--postgis` 或完整分字段 `--postgis-host/port/dbname/user/password + (table|sql)`
- **Mongo 输出**：`--mongo` 或完整分字段 `--mongo-host/port/dbname/username/password/auth-source/collection`
- **输出目标**：Mongo 已启用时 `-o` 可选；仅需要 Mongo 时可不提供 `-o`
